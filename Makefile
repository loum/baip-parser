PY=/usr/bin/python
NOSE=/usr/bin/nosetests1.1 -s -v --with-xunit --with-coverage --cover-erase --cover-package baip_parser
NOSE_ENV=.env/bin/nosetests -s -v --with-xunit --with-coverage --cover-erase --cover-package baip_parser
GIT=/usr/bin/git
...
uninstall:
	$(RPM) -e python-baip-parser

install:
	$(RPM) -ivh dist/python-baip-parser-?.?.?-?.noarch.rpm

upgrade:
	$(RPM) -Uvh dist/python-baip-parser-?.?.?-?.noarch.rpm
